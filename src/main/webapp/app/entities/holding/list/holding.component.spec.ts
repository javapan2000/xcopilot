...
import { sampleWithRequiredData } from '../holding.test-samples';
...
describe('Holding Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.holdings()[0]).toEqual(expect.objectContaining({ id: 20233 }));
  });

  describe('trackId', () => {
    it('Should forward to holdingService', () => {
      const entity = { id: 20233 };
      jest.spyOn(service, 'getHoldingIdentifier');
      const id = comp.trackId(entity);
      expect(service.getHoldingIdentifier).toHaveBeenCalledWith(entity);
