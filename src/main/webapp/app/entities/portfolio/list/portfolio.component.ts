...
export class PortfolioComponent implements OnInit {
  ...
  trackId = (item: IPortfolio): number => this.portfolioService.getPortfolioIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.portfolios.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IPortfolio[] | null): IPortfolio[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const portfoliosNew = this.portfolios();
      if (data) {
        for (const d of data) {
          if (portfoliosNew.some(op => op.id === d.id)) {
            portfoliosNew.push(d);
          }
        }
      }
      return portfoliosNew;
    }
    return data ?? [];
  }
