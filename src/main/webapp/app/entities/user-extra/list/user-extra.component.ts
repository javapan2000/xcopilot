...
export class UserExtraComponent implements OnInit {
  ...
  userExtras = signal<IUserExtra[]>([]);
  isLoading = false;

  sortState = sortStateSignal({});

  public readonly router = inject(Router);
  protected readonly userExtraService = inject(UserExtraService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IUserExtra): number => this.userExtraService.getUserExtraIdentifier(item);
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (this.userExtras().length === 0) {
            this.load();
          } else {
            this.userExtras.set(this.refineData(this.userExtras()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.userExtras.set(this.refineData(dataFromBody));
  }
